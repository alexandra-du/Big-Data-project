...
#Data2 est notre base de données après nettoyage des valeurs incohérentes

#Séparation des variables catégoriques (intérêt : ne sélectionner que les catégories qui auront une petite p-valeur dans le modèle glm
...
optim<-function(p1,p2,p3,interval,dframe){
  results<-list()
  p1bis=p1
  while(p1bis<1){
    p2bis=p2
    while (p2bis<p1bis){
      p3bis=p3
      while (p3bis<p2bis) {
        confusion.matrix=table(result.table(p1bis,p2bis,p3bis,data),dframe$value2)
...
```{r}
#Plot interactive 3D graph of p1 p2 p3 and precision
