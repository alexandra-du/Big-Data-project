...
<h2><font color="teal">3.3) Performance of the model on the test data set</font></h2>
```{r}
#test data
...
qda.fit <- qda(Value~ Doors, data = data3)
qda.fit
qda.class = predict (qda.fit,dtest)$class
m<-table (qda.class, dtest$Value)
m
sum(diag(m))/nrow(dtest)
```
...
```{r}
# 3 attributes (buying price, maintenance price, doors)
dtrain.knn3=data.frame(dtrain.knn$Buying_price,dtrain.knn$Maintenance_price.knn,dtrain.knn$Doors.knn,dtrain.knn$Value.knn)
dtest.knn3=data.frame(dtest.knn$Buying_price,dtest.knn$Maintenance_price.knn,dtest.knn$Doors.knn,dtest.knn$Value.knn)
dtrValue3=dtrain.knn3[,4]
dteValue3=dtest.knn3[,4]
results<-list()

for (k in 1:20) {
knn.pred3=knn(dtrain.knn3[,1:3],dtest.knn3[,1:3],dtrValue3,k)
table.test.knn3=table (knn.pred3,dteValue3)
p3=sum(diag(table.test.knn3))/sum(table.test.knn3)
print(p3)
results=c(results,list(c(k,p3)))
...
```{r}
# 3 attributes (maintenance price, doors, luggage boot)
dtrain.knn4=data.frame(dtrain.knn$Maintenance_price.knn,dtrain.knn$Doors.knn,dtrain.knn$Lug_boot.knn,dtrain.knn$Value.knn)
dtest.knn4=data.frame(dtest.knn$Maintenance_price.knn,dtest.knn$Doors.knn,dtest.knn$Lug_boot.knn,dtest.knn$Value.knn)
dtrValue4=dtrain.knn4[,4]
dteValue4=dtest.knn4[,4]
results<-list()

for (k in 1:20) {
knn.pred4=knn(dtrain.knn4[,1:3],dtest.knn4[,1:3],dtrValue4,k)
table.test.knn4=table (knn.pred4,dteValue4)
p4=sum(diag(table.test.knn4))/sum(table.test.knn4)
results=c(results,list(c(k,p4)))
